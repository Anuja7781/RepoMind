...
function buildNodes(analysis: ArchitectureAnalysis): ArchitectureNode[] {
  const incoming = new Map(analysis.components.map(component => [component.name, 0]))
  const outgoing = new Map(analysis.components.map(component => [component.name, 0]))
  const evidence = new Map(analysis.components.map(component => [component.name, 0]))
  ...
  const queue = analysis.components.filter(component => !remaining.get(component.name)).map(component => component.name)
  queue.forEach(name => levels.set(name, 0))
  while (queue.length) {
    const source = queue.shift()!
    analysis.relationships.filter(edge => edge.source_component === source).forEach(edge => {
  ...
  analysis.components.forEach(component => {
    if (!levels.has(component.name)) levels.set(component.name, analysis.relationships.length ? 0 : Math.max(0, GROUP_ORDER.indexOf(component.component_type)) )
  })
  const rows = new Map<number, ArchitectureComponent[]>()
  analysis.components.forEach(component => rows.set(levels.get(component.name)!, [...(rows.get(levels.get(component.name)!) ?? []), component]))
...
export default function ArchitectureGraph({ analysis }: { analysis: ArchitectureAnalysis }) {
  ...
  const positionedEdges = analysis.relationships.flatMap((relationship, index) => {
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center gap-4 text-xs font-mono text-gray-500">
      ...
      <span>Relationships: <strong className="text-gray-300">{analysis.relationships.length}</strong></span>
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
      <div className="relative overflow-auto rounded-xl border border-white/[0.06] bg-[#07070e]">
        {analysis.relationships.length === 0 && <div className="absolute left-4 right-4 top-4 z-10 rounded-lg border border-cyan-400/20 bg-[#0b1220]/95 px-4 py-3"><p className="text-xs font-medium text-cyan-200">Architecture components recovered, but no evidence-backed dependency relationships were found.</p><p className="mt-1 text-[11px] text-gray-500">No arrows are drawn because no reliable connection was recovered.</p></div>}
      ...
      <aside className="rounded-xl border border-white/[0.06] bg-[#0b0b14] p-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 text-xs"><span className="text-gray-500">Confidence<strong className="block text-gray-200">{analysis.confidence}</strong></span><span className="text-gray-500">Entry point<strong className="block truncate text-gray-200">{analysis.entry_point ?? "Not recovered"}</strong></span><span className="text-gray-500">Primary language<strong className="block text-gray-200">{analysis.primary_language ?? "Unknown"}</strong></span><span className="text-gray-500">Evidence<strong className="block text-gray-200">{analysis.evidence.length} signals</strong></span></div>
